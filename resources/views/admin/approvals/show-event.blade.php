...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Evento</p>
            <h2 class="font-display text-3xl text-slate-900">{{ $event->title }}</h2>
            <p class="text-sm text-slate-500">{{ $event->start_at?->format('d/m/Y H:i') }}@if($event->end_at) ate {{ $event->end_at->format('d/m/Y H:i') }}@endif</p>
        </div>
    ...
    <div class="totem-card p-6 space-y-6">
        <div class="flex flex-wrap items-center gap-3">
            <span class="px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-xs uppercase tracking-[0.2em]">{{ strtoupper($event->status) }}</span>
            <span class="text-xs text-slate-400">Operador: {{ $event->creator?->name ?? 'Nao informado' }}</span>
            @if ($event->location)
                <span class="text-xs text-slate-400">Local: {{ $event->location }}</span>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] gap-6">
            <div class="rounded-2xl overflow-hidden bg-slate-100">
                ...
                @else
                    <div class="h-72 bg-gradient-to-br from-senac-blue/20 to-senac-orange/20"></div>
                @endif
            </div>
            <div>
                <p class="text-xs uppercase tracking-[0.2em] text-slate-400 mb-2">Descrição</p>
                <p class="text-sm text-slate-600 leading-relaxed">{!! nl2br(e($event->description)) !!}</p>
            </div>
        </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                    @foreach ($event->images as $image)
                        <img src="{{ asset('storage/' . $image->path) }}" alt="Galeria {{ $event->title }}" class="h-32 w-full object-cover rounded-xl">
                    @endforeach
                </div>
            </div>
        @endif
